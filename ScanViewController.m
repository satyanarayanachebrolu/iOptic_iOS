...
- (void)captureResult:(ZXCapture *)capture result:(ZXResult *)result {
  if (!result) return;
    ...
    NSLog(@"result which is encoded:%@",result.text);


  // We got a result. Display information about the result onscreen.
//  NSString *formatString = [self barcodeFormatToString:result.barcodeFormat];
//    NSData *nsdataFromBase64String = [[NSData alloc]
//                                      initWithBase64EncodedString:result.text options:0];
//    
//    // Decoded NSString from the NSData
//    NSString *base64Decoded = [[NSString alloc]
//                               initWithData:nsdataFromBase64String encoding:NSUTF8StringEncoding];
   // NSLog(@"decodedString:%@",base64Decoded);
//  NSString *display = [NSString stringWithFormat:@"Scanned!\n\nFormat: %@\n\nContents:\n%@\nLocation: %@", formatString, result.text, location];
    // Convert JSON to dictionary
    
    NSData *decodedData = [[NSData alloc] initWithBase64EncodedString:result.text options:0];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    detailVC.name = self.selectedPrescriptionName;
    detailVC.currentPrescriptionDict = self.currentPrescription;
    NSError *error = nil;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:self.currentPrescription                                                                options:NSJSONWritingPrettyPrinted error:&error];
    detailVC.currentPrescriptionJSON = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
